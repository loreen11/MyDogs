...
class Form extends Component{
  constructor() {
    super()
    this.state = { uname: "", text: "",interestAdoption:"" }
  }
  ...
  onSubmit = (event) =>
  {
    ...
    .then(json => {
      alert(json.message)
      this.setState({ uname: "", text: "",interestAdoption:"" })
    })
  }

  onChangeInput  = (event) => {
    this.setState({ interestAdoption: event.target.value })
  }
  onChangeText  = (event) => {
    ...
    render() {
      ...
      const Items = data.map((singleItem) => {
          if (singleItem.element==="input")
            return <Input onChange={this.onChangeInput} value={this.state.interestAdoption} key={singleItem.id} item={singleItem} />
